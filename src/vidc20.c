...
void
vidc_get_snapshot(VIDCStateSnapshot *snapshot)
{
	if (snapshot == NULL) {
		return;
	}
	...
	snapshot->palindex = vidc.palindex;
	memcpy(snapshot->palette, vidc.palette, sizeof(vidc.palette));
	snapshot->border_colour = vidc.border_colour;
	memcpy(snapshot->cursor_palette, vidc.cursor_palette, sizeof(vidc.cursor_palette));
	snapshot->hdsr = vidc.hdsr;
	snapshot->hcsr = vidc.hcsr;
	snapshot->hder = vidc.hder;
	snapshot->vdsr = vidc.vdsr;
	snapshot->vcsr = vidc.vcsr;
	snapshot->vcer = vidc.vcer;
	snapshot->vder = vidc.vder;
	snapshot->b0 = vidc.b0;
	snapshot->b1 = vidc.b1;
	snapshot->bit8 = vidc.bit8;

	if (vidc.hder > vidc.hdsr) {
		snapshot->screen_width = vidc.hder - vidc.hdsr;
	}
	if (vidc.vder > vidc.vdsr) {
		snapshot->screen_height = vidc.vder - vidc.vdsr;
	}
	if (vidc.hcsr >= vidc.hdsr) {
		snapshot->cursor_x = vidc.hcsr - vidc.hdsr;
	}
	if (vidc.vcsr >= vidc.vdsr) {
		snapshot->cursor_y = vidc.vcsr - vidc.vdsr;
	}
	if (vidc.vcer >= vidc.vcsr) {
		snapshot->cursor_height = vidc.vcer - vidc.vcsr;
	}
}
