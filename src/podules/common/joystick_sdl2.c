/*
  SDL2 joystick host pathway (ported from Arculator).

  Reusable backend for joystick/gamepad podules: enumerates and polls host
  joysticks into joystick_state[]. NOT currently compiled into the build - it is
  kept here ready to use. To activate it, add a joystick device podule (which
  must define joystick_get_max_joysticks/axis_count/button_count/pov_count),
  then add this file to src/CMakeLists.txt and link SDL2 into rpcemu_core.

  Note: the only Arculator joystick devices (morley_uap, aka10) are Archimedes
  I/O cards whose ADC lives in MEMC space, which the Risc PC bus does not decode,
  so they cannot read analogue axes on RPCEmu.
 */
